            ...
            try:
                ...
                if 'Item' in response:
                    ...
                    dummy_users.append({
                        ...
                        "discord_display_name": user.get("app_username", user.get("discord_username", f"テストユーザー{i}")),
...
def dummy_login(event: dict, context: object) -> dict:
    ...
    try:
        ...
        if not user_id or not password:
            return create_error_response(400, "user_id and password are required")
        ...
        # ダミーアカウントかどうか確認
        if not user.get("is_dummy", False):
            return create_error_response(400, "Not a dummy account")
        
        # パスワード確認（ダミーユーザーは固定パスワード）
        # user_idから数字部分を抽出してパスワードを生成
        ...
        return create_success_response({
            ...
            "user": {
                "user_id": user_id,
                "discord_username": user.get("discord_username"),
                "discord_display_name": user.get("app_username", user.get("discord_username")),
                "rate": int(user.get("rate", 1500)),
...
def validate_dummy_token(token: str) -> dict[str, any]:
    ...
    try:
        ...
        if not (unverified_payload.get("is_dummy") and
                unverified_payload.get("user_id", "").startswith("dummy|discord|")):
            return {}

        # It's a dummy token, so decode it with proper validation but without audience check
        payload = jwt.decode(
            token,
            DUMMY_JWT_SECRET,
            algorithms=["HS256"],
        ...
        if payload.get("is_dummy") and payload.get("user_id", "").startswith("dummy|discord|"):
